...
class Ranking:
    ...
    def get_ranking(self, address, distance, min_price, 
        ...
        aliases = self.alias_dictionary([i[0] for i in self.distance_ranking],[i[0] for i in self.availability_ranking], 0.25)

        return self.get_ranking_list(self.create_ranking_df(aliases), n_neighbourhood).index.values
    ...
    def get_ranking_list(self, df, count=3):
        ...
        df_normalized['Ranking'] = df.apply(lambda x: (x['Distance'] * self.distance_weight) + (x['Availability'] * self.availability_weight)
         + (x['Security'] * self.security_weight * -1), axis=1)
        df_normalized = df_normalized.sort_values('Ranking', ascending=False)
        print(df_normalized)
        return df_normalized.head(count)
